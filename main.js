...
const proccesTxtW = (userNumber, secretNumber) => {
  if (lang == "en") {
    if (userNumber > secretNumber) {
      return "🚫 Number is less then yours!";
    } else {
      return "🚫 Number is more then yours!";
    }
  } else {
    if (userNumber > secretNumber) {
      return "🚫 Число меньше введённого";
    } else {
      return "🚫 Число больше введённого";
    }
  }
};
...
let whichBg = Math.trunc(Math.random() * 10) + 1;
let bgs = [
  "bg.jpg",
  "bg2.jpg",
  "bg3.jpg",
...
btn.addEventListener("click", () => {
  ...
  if (score != 0 && gameEnded == false) {
    if (!userNum) {
      ...
      procces.textContent = proccesTxtW(userNum, secretNum);
      document.querySelector(".secretNum_bg").style.boxShadow =
